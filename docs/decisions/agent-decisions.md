...
| 49 | Alta | El workflow de CI mezclaba dos estrategias E2E incompatibles: Playwright ya levanta `next dev` en `3200`, pero el job además hacía `npm start` en `3000` con `sleep 5` | Se refactorizó `.github/workflows/ci-cd.yml` para centralizar variables (`NODE_VERSION`, `APP_PORT`, `FALLBACK_DATABASE_URL`), usar fallback local de `DATABASE_URL` en CI y dejar que Playwright gestione por sí solo el web server de E2E. |
