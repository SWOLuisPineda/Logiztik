...
| 51 | Alta | Nuevo fallo E2E `P1013`: `datasource.url` inválida porque el job tomó una URL no `file:` (host Postgres sin protocolo) mientras Prisma del repo usa `provider = "sqlite"` | Se fijó `DATABASE_URL=file:./prisma/dev.db` explícitamente en los jobs `build` y `test-e2e` para aislar CI de secrets externos incompatibles con el proveedor SQLite del esquema actual. |
| 52 | Media | Se definió aplicar enfoque híbrido por fases (Opción B): CI pre-merge con SQLite local y validación post-deploy Azure en fase posterior | Se dejó explícito en `.github/workflows/ci-cd.yml` que `build`/`test-e2e` usan SQLite local en CI y `deploy` mantiene publicación a Azure en `main`; la validación remota post-deploy queda planificada para siguiente fase. |
